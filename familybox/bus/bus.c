...
uint8_t cpu_read(Nes *n, uint16_t addr)
{
    addr &= 0xFFFF;
    if (addr >= 0x8000)
        return mapper0_cpu_read(n, addr);
    if (addr < 0x2000)
        return n->ram[addr & 0x7FF];
    if (addr < 0x4000)
        return ppu_reg_read(n, 0x2000 + (addr & 7));
    if (addr == 0x4016)
        return controller_read(n);
    if (addr == 0x4015)
        return apu_reg_read(n, addr);
    if (addr >= 0x6000)
        return n->cart.wram[addr - 0x6000];
    return 0;
}
