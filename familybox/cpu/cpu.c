...
void cpu_trigger_nmi(Nes *n) { n->cpu.nmi_pending = 1; }
...
{
    if (cond)
        n->cpu.r.p = (uint8_t)(n->cpu.r.p | flag);
    else
        n->cpu.r.p = (uint8_t)(n->cpu.r.p & ~flag);
}
...
static uint16_t am_abx(Nes *n, int *pcross)
{
    ...
    if (pcross)
        *pcross = ((base & 0xFF00) != (addr & 0xFF00));
    return addr;
}
...
static void branch(Nes *n, int cond, int *cycles)
{
    ...
    if (cond)
    {
        ...
        (*cycles)++;
        if ((old & 0xFF00) != (n->cpu.r.pc & 0xFF00))
            (*cycles)++;
    }
}
...
int cpu_tick(Nes *n)
{
    ...
    case 0x6D:
        ...
        break;
    case 0x7D:
        addr = am_abx(n, &pcross);
        adc(n, cpu_read(n, addr));
        cycles = 4 + pcross;
        break;
    case 0x79:
        addr = am_aby(n, &pcross);
        adc(n, cpu_read(n, addr));
        cycles = 4 + pcross;
        break;
    case 0x61:
    ...
    case 0x90:
        branch(n, !(r->p & F_C), &cycles);
        break; /* BCC */
    case 0xB0:
        branch(n, (r->p & F_C) != 0, &cycles);
        break; /* BCS */
    case 0xF0:
        branch(n, (r->p & F_Z) != 0, &cycles);
        break; /* BEQ */
    case 0x30:
        branch(n, (r->p & F_N) != 0, &cycles);
        break; /* BMI */
    case 0xD0:
        branch(n, !(r->p & F_Z), &cycles);
        break; /* BNE */
    case 0x10:
        branch(n, !(r->p & F_N), &cycles);
        break; /* BPL */
    case 0x50:
        branch(n, !(r->p & F_V), &cycles);
        break; /* BVC */
    case 0x70:
        branch(n, (r->p & F_V) != 0, &cycles);
        break; /* BVS */
    ...
    case 0xC6:
        addr = am_zp(n);
        {
            uint8_t v = (uint8_t)(cpu_read(n, addr) - 1);
            cpu_write(n, addr, v);
            set_zn(n, v);
        }
        cycles = 5;
