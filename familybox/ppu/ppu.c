...
static void render_scanline(Nes* n, int y) {
    ...
    if (show_bg(p)) {
        ...
        for (int tile_x = start_tile; tile_x < 33; tile_x++) {
            ...
            for (int px = 0; px < 8; px++) {
                ...
                bg_pix[sx] = ci;
                uint16_t paddr = (ci == 0) ? (uint16_t)0x3F00
                                           : (uint16_t)(0x3F00 + pal * 4 + ci);
        ...
        for (int x = 0; x < 256; x++) put_pixel(n, x, y, backdrop);
    }
    ...
    if (show_sp(p)) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            int rel = y - start_y;
            if (rel < 0 || rel >= height) continue;

            if (attr & 0x80) rel = height - 1 - rel;
            uint16_t pat;
            int row = rel;
            if (height == 16) {
                int t = tile & 0xFE;
            ...
            for (int px = 0; px < 8; px++) {
                ...
                if (!show_left_sp(p) && x < 8) continue;
                int bit = (attr & 0x40) ? px : (7 - px);
                uint8_t ci = (uint8_t)(((lo >> bit) & 1) | (((hi >> bit) & 1) << 1));
                if (ci == 0) continue;
                if (sp_claimed[x]) continue;
                sp_claimed[x] = 1;
