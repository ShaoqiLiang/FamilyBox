...
int nes_run_frame(Nes *n, uint8_t *rgb, int16_t *pcm, int max_samples)
{
    ...
    for (int sl = 0; sl < n->timing.scanlines_per_frame; sl++)
    {
        ...
        while (dots_done < PPU_DOTS_PER_SCANLINE)
        {
            ...
            {
                if (n->ppu.scanline == 0)
                    n->v_samples[0] = n->ppu.v;
                if (n->ppu.scanline == 20)
                    n->v_samples[1] = n->ppu.v;
                if (n->ppu.scanline == 40)
                    n->v_samples[2] = n->ppu.v;
                if (n->ppu.scanline == 100)
                    n->v_samples[3] = n->ppu.v;
            }
            if (ppu_step(n, step))
            {
                cpu_trigger_nmi(n);
    ...
    if (n->ppu.frame % 60 == 0)
    {
        ...
        FB_DBG("t=%lums frame=%d pc=%04X A=%02X X=%02X Y=%02X P=%02X\n", fb_now_ms(), n->ppu.frame, n->cpu.r.pc,
               n->cpu.r.a, n->cpu.r.x, n->cpu.r.y, n->cpu.r.p);
        FB_DBG("  ctrl=%02X mask=%02X status=%02X v=%04X t=%04X fineX=%d sl=%d\n", n->ppu.ctrl, n->ppu.mask,
               n->ppu.status, n->ppu.v, n->ppu.t, n->ppu.x, n->ppu.scanline);
        FB_DBG("  v@0/20/40/100=%04X/%04X/%04X/%04X  s0hits=%d last_s0_sl=%d nmi=%d\n", n->v_samples[0],
               n->v_samples[1], n->v_samples[2], n->v_samples[3], n->s0_hit_count, n->last_s0_scanline, n->nmi_count);
        FB_DBG("  oam0=%02X %02X %02X %02X  pal0-3=%02X %02X %02X %02X  fb_nz=%d\n", oam[0], oam[1], oam[2], oam[3],
               pal[0], pal[1], pal[2], pal[3], nz);
        FB_DBG("  scroll_writes=%d wrlog=%d%s  ram073F/0740/0778=%02X %02X %02X\n", n->scroll_writes, n->wrlog_n,
               n->wrlog_full ? "+" : "", n->ram[0x73F], n->ram[0x740], n->ram[0x778]);
        FB_DBG("  nt0[0..7]=%02X %02X %02X %02X %02X %02X %02X %02X\n", n->ppu_mem.nametable[0],
               n->ppu_mem.nametable[1], n->ppu_mem.nametable[2], n->ppu_mem.nametable[3], n->ppu_mem.nametable[4],
               n->ppu_mem.nametable[5], n->ppu_mem.nametable[6], n->ppu_mem.nametable[7]);
        int start = n->wrlog_n > 8 ? n->wrlog_n - 8 : 0;
        for (int i = start; i < n->wrlog_n; i++)
        {
            FB_DBG("  wr[%d] kind=%d addr=%04X v=%02X sl=%d cy=%d\n", i, n->wrlog[i].kind, n->wrlog[i].addr,
                   n->wrlog[i].value, n->wrlog[i].scanline, n->wrlog[i].cycle);
        }
        n->wrlog_n = 0;
        n->wrlog_full = 0;
    }
